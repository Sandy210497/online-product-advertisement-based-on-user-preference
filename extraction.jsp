	...
	function process()
	{
		...
		for(var j=0; inputElements[j]; ++j)
		{
     	 	if(inputElements[j].checked)
     	 	{
          		...
          		chearray.push(inputElements[j].value);
          		//che = {inputElements};
          		//che[j]=(inputElements[j].value);
      		}
      	}
		 alert("check :: "+chearray);
		
		var url = "Keyframe?checkbox="+chearray;
		makeRequest(url);
	...
	<div id="demo-1" class="banner-inner">
	 <div class="banner-inner-dott">
		...
		<div class="agile_featured_movies" align="left" style="padding-left: 40px; margin-left: 30px;">
						<!--/comedy-movies-->
		<h3 class="agile_w3_title hor-t">KeyFrame Extraction</h3>
				...
				for(File farr : fff)
				{
					...
					%>
						<br/>	
							<tr>
							<td>
								<input type="checkbox" class="messageCheckbox" id="c1" name="check" value="<%=folder+"$"%><%=filename %>"/>
							</td>
							<td>
								<%=filename %>
							</td>
		...
		<tr>
			...
			<td>
				<input type="button" data-toggle="modal" data-target="#myModal4" value="process" onclick="process()">
			</td>
		</tr>
		</table>
		</form>
